...
test("lists live catalog ids and marks WORKFLOW pins available or missing", () => {
  const catalog = ModelCatalog.createNull(
    HarnessSetup.createNull({ cursorModels: ["cursor-grok-4.6-high", "composer-2.5"] }),
  );

  const reports = catalog.discover();

  expect(reports.find((report) => report.harness === "pi")?.models).toEqual(["openai-codex/gpt-5.6-sol"]);
  expect(reports.find((report) => report.harness === "cursor-agent")?.models).toEqual(["cursor-grok-4.6-high", "composer-2.5"]);
  expect(reports.find((report) => report.harness === "cursor-agent")?.profiles).toEqual([
    { name: "cursor-grok-high", model: "cursor-grok-4.5-high", available: false },
  ]);
  expect(formatModelCatalog(reports)).toContain("profile cursor-grok-high: cursor-grok-4.5-high missing");
...
test("can list one harness and reports a probe failure without inventing ids or hiding Pi", () => {
  ...
  expect(cursor?.error).toMatch(/authentication|logged in/i);
});
