...
import { parseProfileOptions, profileFingerprint } from "../domain/execution-profile.ts";
import type { RegistryDatabase } from "../infrastructure/sqlite/registry-database.ts";

interface JobRow extends Omit<Job, "userNotified" | "agentNotified"> { userNotified: number; agentNotified: number; }
...
export class JobRepository {
  ...
  static create(database: RegistryDatabase): JobRepository {
    return new JobRepository({
      create: (job) => {
        ...
        database.db.query(`INSERT INTO jobs (
          ...
          .run(job.id, job.taskId, job.jobTypeId, job.agentProfileId, job.agentProfileSelectionSource,
            job.parentSessionId, job.parentSessionFile, job.snapshotProvenance, job.profileFingerprint,
            job.profileOptions, job.capabilityProfile, job.harness, job.harnessVersion, job.nativeInvocation,
            job.model, job.effortLevel, job.modelName, job.modelVersion, job.title, job.status, job.progress,
            job.createdAt, job.finishedAt, job.bundlePath, Number(job.userNotified), Number(job.agentNotified));
      },
      ...
      listUnnotifiedSettledForParent: (parentSessionId) => (database.db.query(`SELECT * FROM jobs WHERE parentSessionId = ?
        AND status IN ('completed', 'failed', 'cancelled', 'skipped') AND (userNotified = 0 OR agentNotified = 0)
        ORDER BY finishedAt, createdAt`).all(parentSessionId) as JobRow[]).map(mapJob),
      updateStatus: (id, status, progress, finishedAt) => { database.db.query("UPDATE jobs SET status = ?, progress = ?, finishedAt = ? WHERE id = ?").run(status, progress, finishedAt, id); },
  ...
  static createNull(jobs: Job[] = []): JobRepository {
    ...
    return new JobRepository({
      create: (job) => { validateNewJobSnapshot(job); if (records.has(job.id)) throw new Error(`Duplicate job: ${job.id}`); records.set(job.id, structuredClone(job)); },
      get: (id) => clone(records.get(id)),
      listForTask: (taskId) => [...records.values()].filter((job) => job.taskId === taskId).map((job) => structuredClone(job)).sort((a, b) => a.createdAt.localeCompare(b.createdAt)),
      listUnnotifiedSettledForParent: (parentSessionId) => [...records.values()]
        ...
        .map((job) => structuredClone(job)).sort((a, b) => (a.finishedAt ?? a.createdAt).localeCompare(b.finishedAt ?? b.createdAt)),
      updateStatus: (id, status, progress, finishedAt) => { const job = records.get(id); if (job) Object.assign(job, { status, progress, finishedAt }); },
  ...
  updateStatus(id: string, status: JobStatus, progress: string | null, finishedAt: string | null = null): void { this.persistence.updateStatus(id, status, progress, finishedAt); }
...
function mapJob(row: JobRow): Job { validatePersistedJobSnapshot(row as unknown as Job); return { ...row, userNotified: Boolean(row.userNotified), agentNotified: Boolean(row.agentNotified) }; }
function validateNewJobSnapshot(job: Job): void {
  const fields = [job.profileFingerprint, job.profileOptions, job.capabilityProfile, job.harnessVersion, job.nativeInvocation];
  if (job.snapshotProvenance !== "current" || fields.some((value) => value == null)) throw new Error(`New job ${job.id} requires a complete current execution snapshot`);
...
function validateCompleteSnapshot(job: Job): void {
  ...
  if (!["read-only", "code", "test"].includes(job.capabilityProfile!)) throw new Error(`Job ${job.id} has an invalid capability snapshot`);
  ...
  if (Object.keys(value).sort().join(",") !== "args,executable" || typeof value.executable !== "string" || !value.executable.startsWith("/") || !Array.isArray(value.args) || !value.args.every((arg) => typeof arg === "string")) throw new Error(`Job ${job.id} has an invalid native invocation snapshot`);
  const harness = job.harness as "pi" | "cursor-agent";
  const base = { name: job.agentProfileId, harness, model: job.model, options: parseProfileOptions(job.profileOptions!) };
