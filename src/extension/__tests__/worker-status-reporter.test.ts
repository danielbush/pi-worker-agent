...
test("emits manager-visible status from nulled worker state", async () => {
  ...
  const registry = Registry.createNull({
    ...
    jobs: [{
      ...
      bundlePath: "/null-worker-agent/tasks/task_demo/jobs/job_plan",
      userNotified: true,
      agentNotified: true,
    }],
    workerSessions: [{
  ...
  expect(status).toEqual({
    ...
    jobs: [{
      ...
      progress: "Planning completed",
      workerSessionId: "session_plan",
      requestPath: "/null-worker-agent/tasks/task_demo/jobs/job_plan/request.md",
      eventsPath: "/null-worker-agent/tasks/task_demo/jobs/job_plan/worker-sessions/session_plan/events.jsonl",
