...
- `WORKFLOW.md` gives policy meaning to job-type and dependency labels and defines the possible job sequences, evaluation, revision, inspection, merge offers, and approval points. It should state the job types and execution requirements its workflows expect. SQLite `jobDependencies` records the concrete job links and relationships in the flow that actually occurred; it does not define the policy.
...
Use `worker_manage_project_file` to create, replace, patch, or delete policy-owned text files under a registered project without enabling development mode. Always read `PROJECT.md` and the relevant target section before editing. Prefer an exact, uniquely matching patch for localized changes so large files do not enter the manager context; use whole-file replacement only when necessary. Whole-file updates and deletes require exact inspected content, while patches require exact inspected target text. Preserve user comments and report the returned focused diff. The tool's hard boundary is registered-project containment, not the meaning or format of policy files.

Do not infer a named workflow from slash commands or implementation code. Names such as **Demo task** refer to sections in `$DATA_ROOT/WORKFLOW.md`; they are manager policy, not commands. Apply the selected workflow one transition at a time, evaluating each completed job before creating the next one and recording its actual dependency and relationship in `jobDependencies`. Re-read these files on later turns because the user may revise them.
