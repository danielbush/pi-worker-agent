...
- **feat: Decouple workflow job types from execution permissions**
  ...
  - Default job-profile assignments should select harness, logical model, normalized effort, and trusted capability profile; tasks can override those defaults per job purpose.
  - Keep logical model and effort independent, then let each harness adapter map them to provider syntax when version or effort is baked into a model designation.
  - Snapshot the resolved profile and harness-specific mapping on each job for auditability.
