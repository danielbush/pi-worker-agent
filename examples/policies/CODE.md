...
- Give each class or value object its own source file named after that construct, such as `ProjectCollectionPaths` in `project-collection-paths.ts` (TS/JS) or `project_collection_paths.py` (Python). Name files after the primary domain construct rather than a broad topic, and avoid generic names such as `types.ts`/`utils.py`. Exceptions: private helpers used only by their host (underscore-prefixed in Python), and trivially coupled members such as an enum or error type used by exactly one class, which may live with that class. When in doubt, split.
