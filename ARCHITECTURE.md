...
When a detached worker settles, the completion monitor injects its durable result and triggers a manager turn. The manager re-reads policy, evaluates the result, and chooses the next transition. The runner does not encode or create workflow successors. If the final required job is accepted, the manager explicitly completes the task through a narrow tool; application code rejects completion while jobs remain active or when the final job was unsuccessful.
